...
The primary simplifications and assumptions that the SPM model makes concern ignoring the effect of the electrolyte transport effects, as well as modeling the entire solid electrode material as a single particle into which intercalation and deinteracalation take place. \\

These simplifications work well for moderate to low charge/discharge rates as well as assuming that the conductivity of the eletrolyte is effectively negligable. Since the process of lithium transport possess the slowest dynamics in the battery, we can significantly reduce the computaional complexity by ignoring these effects. However, in the event that these effects are import for the problem being analyzed, there is another version of the SPM model, named the Single Particle Model with Electrolyte dynamics (SPMe) which retains the dyanmics of the electrolyte to preserve high predictive accuracy.




 and therefore the presents itself as the prime candidate for





 The SPM model takes this fact and reduces each electrode, of the battery, to an individual sphere of active material that is presumed to be representative of the average dynamics of the electrodes. It should be notes that in this model, another simplification is made whereby the electolyte concentration and dynamics are discarded. The SPMe elevates the SPM model by including these electrolyte dynamics back into the model, allowing this model to capture the nonlinearities which are present due to transport behavior of the electrolyte.
...
\section{Conclusion}
While a complete derivation of the microscale model for electrochemical batteries is far involved and nuanced than the material presented in this paper. The hope has been that this paper
